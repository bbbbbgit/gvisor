...
func TestOne(t *testing.T) {
	...
	for _, dn := range []*dockerutil.DockerNetwork{ctrlNet, testNet} {
		for {
			if err := createDockerNetwork(dn); err != nil {
				...
				t.Log("sleeping 100ms and will try creating docker network again")
	...
	for _, d := range []struct {
		*dockerutil.Docker
		...
		if err := d.Docker.Create(dockerutil.RunOpts{Image: "packetimpact", CapAdd: []string{"NET_ADMIN"}, Extra: []string{"--sysctl", "net.ipv6.conf.all.disable_ipv6=0", "--rm"}, Foreground: true, Pty: func(_ *exec.Cmd, _ *os.File) {}}); err != nil {
			t.Fatalf("unable to create container %s: %s", d.Docker.Name, err)
		}
		defer d.CleanUp()
		for _, dn := range []*dockerutil.DockerNetwork{ctrlNet, testNet} {
			...
			if err := dn.Connect(d.Docker, "--ip", ip.String()); err != nil {
				t.Fatalf("unable to connect container %s to network %v: %s", d.Docker.Name, dn, err)
			}
		}
		if err := d.Docker.Start(); err != nil {
			t.Fatalf("unable to start container %s: %s", d.Docker.Name, err)
		}
	}
	...
	//go dut.Exec(dockerutil.RunOpts{}, containerPosixServerBinary, "--ip="+ip.String(), "--port="+ctrlPort)
	//if _, err := dut.WaitForOutput("Server listening.*\n", 60*time.Second); err != nil {
	//	t.Fatalf("%s on %v never listened: %s", containerPosixServerBinary, dut, err)
	//}

	dutTestDevice, dutDeviceInfo, err := deviceByIP(dut, addressInSubnet(dutAddr, *testNet.Subnet))
	if err != nil {
		t.Fatalf("can't find deviceInfo for on container %s: %s", dut.Name, err)
	}
	testbenchTestDevice, testbenchDeviceInfo, err := deviceByIP(testbench, addressInSubnet(testbenchAddr, *testNet.Subnet))
	if err != nil {
		t.Fatalf("can't find deviceInfo for on container %s: %s", testbench.Name, err)
	}
	...
	if err != nil {
		t.Fatalf("unable to run %s on container %s: %s", snifferArgs[0], testbench.Name, err)
	}
	...
	if err != nil {
		t.Fatalf("unable to exec %s on container %s: %s", testArgs, testbench.Name, err)
	}
